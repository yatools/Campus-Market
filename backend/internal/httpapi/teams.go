...
func (s *Server) writeTeamPayload(w http.ResponseWriter, r *http.Request, teamID int64, viewer *User) error {
	currentTeam, err := s.loadTeam(r.Context(), s.DB, teamID)
	if err != nil {
		return err
	}
	payload, err := s.teamPayload(r.Context(), currentTeam, viewer)
	if err != nil {
		return err
	}
