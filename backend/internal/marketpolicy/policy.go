...
func ListingCancellable(status string) bool { return status == "available" || status == "reserved" }
...
func RequestAcceptable(transactionStatus, listingStatus string) bool {
	return transactionStatus == "requested" && listingStatus == "available"
}
...
func Confirmable(status string) bool { return status == "reserved" || status == "completed" }
