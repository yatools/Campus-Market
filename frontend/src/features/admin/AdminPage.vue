...
const overview = ref<AdminOverview | null>(null), users = ref<User[]>([]), cases = ref<ModerationCase[]>([]), appeals = ref<AdminAppeal[]>([]), feedback = ref<Feedback[]>([]), backups = ref<BackupJob[]>([]), auditRows = ref<AuditLog[]>([])
const marketDisputes = ref<MarketDispute[]>([]), systemHealth = ref<AdminSystemHealth | null>(null)
...
async function load() {
  ...
  try {
    ...
    const workspace = await adminApi.moderationWorkspace()
    overview.value = workspace.overview; users.value = workspace.users.items; cases.value = workspace.cases.items; appeals.value = workspace.appeals.items; feedback.value = workspace.feedback.items; marketDisputes.value = workspace.marketDisputes.items
    if (auth.isAdmin) {
      const administrator = await adminApi.administratorWorkspace()
      Object.assign(settings, administrator.settings); backups.value = administrator.backups.items; auditRows.value = administrator.auditLogs.items; creditRules.value = administrator.creditRules; systemHealth.value = administrator.health; systemHealthRefreshedAt.value = new Date()
      teamGames.value = administrator.games.items; gameSubmissions.value = administrator.submissions.items; campusServices.value = administrator.services.items
      marketCategories.value = administrator.categories.items; marketLocations.value = administrator.locations.items
    }
  } finally {
    loading.value = false
  }
...
async function refreshSystemHealth() {
  ...
  try {
    systemHealth.value = await adminApi.health()
...
async function decide(item: ModerationCase, decision: string) {
  ...
  await adminApi.decideModeration(item.id, { decision, note, respondent_id }); success.value = '审核已完成并记录审计日志'; await load()
}
async function saveUser(item: User) {
  const reason = prompt('请输入本次修改原因：', '')
  if (!reason) return
  await adminApi.updateUser(item.id, { role: item.role, campus_identity: item.campus_identity, status: item.status, credit: Number(item.credit), reason }); success.value = '用户状态已更新'; await load()
}
async function punish(item: User) {
  ...
  await adminApi.createPenalty({ user_id: item.id, violation_type, result, rule, credit_delta })
  success.value = '处罚已记录、通知责任人并进入可申诉流程'; await load()
}
async function decideAppeal(item: AdminAppeal, status: string) { const note = prompt('处理说明：', ''); if (note === null) return; await adminApi.decideAppeal(item.id, { status, note }); await load() }
async function decideFeedback(item: Feedback, status: string) { const admin_note = prompt('给提交者的说明：', ''); if (admin_note === null) return; await adminApi.decideFeedback(item.id, { status, admin_note, reward_xp: status === 'accepted' ? 20 : 0 }); await load() }
async function decideMarketDispute(item: MarketDispute, decision: 'completed' | 'cancelled') { const note = prompt('请输入交易纠纷裁决说明：', ''); if (note === null || !note) return; await adminApi.decideDispute(item.id, { decision, note }); success.value = '交易纠纷已裁决并写入审计日志'; await load() }
async function publishAnnouncement() { await adminApi.publishAnnouncement(announcement); Object.assign(announcement, { title: '', body: '', level: 'normal', audience: 'all' }); success.value = '公告已发布' }
async function createCourseOffering() { await adminApi.createCourseOffering(courseForm.name, courseForm.teacher, courseForm.semester, courseForm.section); Object.assign(courseForm, { name: '', teacher: '', semester: '', section: '' }); success.value = '课程与班次已加入目录' }
async function createCampusService() { await adminApi.createCampusService({ name: serviceForm.name, category: serviceForm.category, manager_user_id: serviceForm.manager_user_id ? Number(serviceForm.manager_user_id) : null }); Object.assign(serviceForm, { name: '', category: '校园服务', manager_user_id: '' }); success.value = '校园服务已加入评分目录'; await load() }
async function editCampusService(item: CampusService) { const manager = prompt('服务管理者用户 ID（留空为未指定）：', '') ; if (manager === null) return; await adminApi.updateCampusService(item.id, { manager_user_id: manager ? Number(manager) : null }); success.value = '服务管理者已更新'; await load() }
async function createMarketOption() {
  await adminApi.createMarketOption(marketOptionForm.kind, { name: marketOptionForm.name, slug: marketOptionForm.slug, sort_order: Number(marketOptionForm.sort_order), active: true })
...
async function saveMarketOption(kind: 'categories' | 'locations', item: AdminMarketOption) {
  await adminApi.updateMarketOption(kind, item.id, { name: item.name, slug: item.slug, sort_order: Number(item.sort_order), active: item.active })
...
async function disableMarketOption(kind: 'categories' | 'locations', item: AdminMarketOption) {
  ...
  await adminApi.deleteMarketOption(kind, item.id)
...
async function decideGame(item: GameSubmission, action: string) {
  ...
  await adminApi.decideGame(item.id, { action, target_game_id, canonical_name, admin_note })
  success.value = '游戏提交已处理'; await load()
}
async function saveSettings() { for (const key of siteSettingKeys.value) await adminApi.saveSetting(key, settings[key] || ''); success.value = '站点设置已保存' }
async function saveCreditRules() { creditRules.value = await adminApi.saveCreditRules({ rules: creditRules.value.rules.map(({ key, value }) => ({ key, value: Number(value) })) }); success.value = '信用规则已保存，新的权限判断和奖惩会立即采用'; await auth.load() }
async function saveAnonymousPool() { const result = await adminApi.saveSetting('anonymous_nickname_pool', settings.anonymous_nickname_pool || ''); settings.anonymous_nickname_pool = result.value; success.value = '匿名昵称池已保存，新分配的树洞身份会采用这份名单' }
async function backup() { await adminApi.requestBackup(); success.value = '备份任务已进入后台队列'; await load() }
onMounted(() => run(load))
</script>
...
<template>
  <section>
    ...
    <p v-if="loading" class="empty-state">正在加载管理工作区…</p>
    <p v-else-if="!auth.canModerate" class="empty-state">当前账号没有审核权限。</p>
    <template v-else>
