...
import ExploreAnnouncements from './ExploreAnnouncements.vue'
import { exploreSectionInfo, isExploreSection, type ExploreSection } from './registry'
import { useAuthStore } from '../../stores/auth'
import type { Announcement, CampusService, CampusServiceRating, LostClaim, MarketOptions, MarketTransaction, QuestionAnswer } from '../../generated/sdk'
import { formatPrice, marketTransactionActions } from '../../market'
import { exploreApi, type ExploreSubmitAction } from './api'
import { emptyExploreForm, isMarketItem, type ExploreForm, type ExploreItemViewModel } from './model'
...
const items = ref<ExploreItemViewModel[]>([])
const currentPage = ref(1)
const pageSize = ref(20)
const total = ref(0)
const loading = ref(true)
const error = ref('')
// Disables the primary submit button while a request is in flight.
const submitting = ref(false)
const modal = ref(false)
const action = ref<ExploreSubmitAction | 'claims'>('create')
const target = ref<ExploreItemViewModel | null>(null)
const detailItem = ref<ExploreItemViewModel | null>(null)
const comments = ref<number | null>(null)
const form = reactive<ExploreForm>(emptyExploreForm())
...
const unmaskHint = computed(() => !auth.user
  ...
  : `观察帖默认打码显示。查看原文需要信用 ${unmaskThreshold.value}，你当前 ${auth.user.credit}，还差 ${Math.max(0, unmaskThreshold.value - auth.user.credit)} 分。`)
...
async function load() {
  ...
  try {
    const [response, services] = await Promise.all([
      exploreApi.list(section.value, currentPage.value),
      section.value === 'handbook' ? exploreApi.campusServices() : Promise.resolve({ items: [] as CampusService[], total: 0 }),
    ])
    ...
    if (section.value === 'listings') marketOptions.value = await exploreApi.marketOptions()
    if (token !== loadToken) return
...
async function openDetail(item: ExploreItemViewModel) { error.value = ''; detailItem.value = section.value === 'questions' ? await exploreApi.question(item.id) : item; comments.value = null }
function closeDetail() { detailItem.value = null; comments.value = null }
function openAnswer(item: ExploreItemViewModel) { if (auth.requireLogin()) { resetForm(); action.value = 'answer'; target.value = item; modal.value = true } }
function openClaim(item: ExploreItemViewModel) { if (auth.requireLogin()) { resetForm(); action.value = 'claim'; target.value = item; modal.value = true } }
async function openClaims(item: ExploreItemViewModel) {
  if (!auth.requireLogin()) return
  resetForm()
  target.value = item
  target.value.claims = (await exploreApi.claims(item.id)).items
  action.value = 'claims'
  modal.value = true
}
function openAppeal(item: ExploreItemViewModel) { if (auth.requireLogin()) { resetForm(); action.value = 'appeal'; target.value = item; modal.value = true } }
function openResponse(item: ExploreItemViewModel) { if (auth.requireLogin()) { resetForm(); action.value = 'respond'; target.value = item; modal.value = true } }
function openListingEdit(item: ExploreItemViewModel) {
  if (!auth.requireLogin()) return
  if (!isMarketItem(item)) {
    error.value = '商品数据不完整，请刷新后重试'
    return
  }
  resetForm()
...
function openEdit(item: ExploreItemViewModel) {
  if (!auth.requireLogin()) return
  resetForm()
  action.value = 'edit'
  target.value = item
...
async function submit() {
  ...
  try {
    if (action.value === 'claims') return
    await exploreApi.submit({
      action: action.value,
      section: section.value,
      targetId: target.value?.id ?? null,
      form: { ...form, attachments: [...form.attachments] },
    })
    modal.value = false
    await auth.load()
    await load()
...
async function accept(answer: QuestionAnswer) { await exploreApi.acceptAnswer(answer.id); await load() }
async function favorite(item: ExploreItemViewModel) { if (auth.requireLogin()) { await exploreApi.favorite(item.id); await load() } }
async function activityJoin(item: ExploreItemViewModel) { if (auth.requireLogin()) { await exploreApi.setActivityMembership(item.id, item.joined); await load() } }
async function cancelActivity(item: ExploreItemViewModel) { if (confirm('确定取消活动并通知所有成员？')) { await exploreApi.cancelActivity(item.id); await load() } }
async function markRead(item: Announcement) { if (auth.requireLogin()) { await exploreApi.readAnnouncement(item.id); item.read = true } }
async function messageSeller(item: ExploreItemViewModel) {
  if (!auth.requireLogin()) return
  const text = prompt('给卖家发送第一条消息：', `你好，请问“${item.title}”还在吗？`)
  if (!text) return
  const result = await exploreApi.messageSeller(item.seller.id, item.id, text)
...
async function requestReservation(item: ExploreItemViewModel) {
  if (!auth.requireLogin()) return
  const message = prompt('给卖家的预订留言：', `希望预订“${item.title}”，可校内面交。`)
  if (message === null) return // 用户取消，不应提交预订
  try {
    await exploreApi.requestReservation(item.id, message)
    await showMyTransactions()
  } catch (e) {
    error.value = e instanceof Error ? e.message : '预订失败'
  }
}
async function showListingTransactions(item: ExploreItemViewModel) { transactionTitle.value = `“${item.title}”的预订申请`; marketTransactions.value = (await exploreApi.listingTransactions(item.id)).items }
async function showMyTransactions() { if (!auth.requireLogin()) return; error.value = ''; transactionTitle.value = '我的买卖记录'; marketTransactions.value = (await exploreApi.myTransactions()).items }
async function transactionAction(item: MarketTransaction, action: 'accept' | 'reject' | 'confirm' | 'cancel') {
  let body: { reason: string } | undefined
  if (action === 'cancel') {
    const reason = prompt('取消原因：', '')
    if (reason === null) return // 取消对话框不应真的取消交易
    body = { reason }
  }
  await exploreApi.transactionAction(item, action, body?.reason)
  await showMyTransactions()
  await load()
}
async function disputeTransaction(item: MarketTransaction) {
  ...
  try {
    ...
    const uploaded = await Promise.all(files.map((file) => exploreApi.upload(file, 'market_dispute')))
    await exploreApi.openDispute(item.id, reason, uploaded.map((entry) => entry.id))
...
async function reviewTransaction(item: MarketTransaction) {
  ...
  try {
    await exploreApi.reviewTransaction(item.id, rating, body)
    await showMyTransactions()
  } catch (e) {
...
async function revealObserve(item: ExploreItemViewModel) {
  if (!auth.requireLogin()) return
  try {
    if (!auth.user?.observe_unmask_agreed) {
      if (!confirm('《吃瓜不扩散协议》：查看原文即代表你承诺不截图、不转发、不扩散帖内信息；违者按规则扣信用分并公示。是否同意并继续？')) return
      const agreement = await exploreApi.acceptObserveAgreement()
      if (auth.user) auth.user.observe_unmask_agreed = agreement.observe_unmask_agreed
    }
    // POST: the endpoint writes an audit record, so it is CSRF-protected server-side.
    const result = await exploreApi.revealObserve(item.id)
...
async function cancelListing(item: ExploreItemViewModel) { if (!confirm('确定取消这件商品？完成交易后不能重新上架。')) return; await exploreApi.cancelListing(item.id); detailItem.value = null; await load() }
async function decideClaim(item: LostClaim, approve: boolean) { if (!target.value) return; await exploreApi.decideClaim(target.value.id, item.id, approve); await openClaims(target.value); await load() }
...
async function openService(service: CampusService) {
  serviceDetail.value = await exploreApi.campusService(service.id)
  Object.assign(serviceForm, { rating: 5, body: '' })
}
async function rateService() {
  if (!serviceDetail.value || !auth.requireLogin()) return
  try {
    await exploreApi.rateCampusService(serviceDetail.value.id, serviceForm)
    await load()
...
async function respondService(rating: CampusServiceRating) {
  ...
  if (!body) return
  await exploreApi.respondToCampusServiceRating(rating.id, { body })
...
function marketName(value: string | { name: string }) { return typeof value === 'string' ? value : value.name }
  ...
  <section class="explore-page-v4" :class="`section-${section}`">
    ...
    <template v-else-if="section === 'questions'">
      ...
      <article v-for="item in items" :key="item.id" class="card qa-card-v4">
        ...
        <div v-for="answer in (item.answers || []).filter((row) => row.id === item.accepted_answer_id)" :key="answer.id" class="best-answer-v4"><b>✅ 最佳答案</b>（{{ answer.author }}）：<RichText :content="answer.body" /><div class="muted">采纳 +20 经验 · 高价值内容可转入生存手册</div></div>
    ...
    <BaseModal v-if="marketTransactions" :title="transactionTitle" wide @close="marketTransactions = null">
      ...
      <div class="service-review-list">
        <article v-for="transaction in marketTransactions" :key="transaction.id" class="card compact">
          ...
          <label v-if="transactionActions(transaction).includes('dispute')" class="notice info">纠纷证据（可选，最多 9 张，先选图再点「发起纠纷」）<input :id="`market-evidence-${transaction.id}`" type="file" accept="image/jpeg,image/png,image/webp" multiple @change="setEvidence(transaction.id, $event)" /></label>
        </article>
