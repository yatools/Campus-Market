...
import type { CampusService, CreditRuleSet, GameSubmission, Page, TeamGame } from '../types'
...
async function load() {
  ...
  if (auth.isAdmin) tasks.push(api('/admin/settings'), api<Page<any>>('/admin/backups'), api<Page<any>>('/admin/audit-logs?page_size=100'), api<CreditRuleSet>('/admin/credit-rules'))
  ...
  if (auth.isAdmin) {
    Object.assign(settings, values[5]); backups.value = values[6].items; auditRows.value = values[7].items; creditRules.value = values[8]
    const [catalog, submissions, services] = await Promise.all([api<{ items: TeamGame[] }>('/team-games'), api<Page<GameSubmission>>('/admin/game-submissions'), api<{ items: CampusService[] }>('/campus-services')])
...
async function punish(item: any) {
  ...
  if (!rule) return
  const credit_delta = Number(prompt('信用分变化（0 到 -1000）：', '-50'))
...
async function decideFeedback(item: any, status: string) { const admin_note = prompt('给提交者的说明：', '') ?? ''; await api(`/admin/feedback/${item.id}/decision`, json('POST', { status, admin_note, reward_xp: status === 'accepted' ? 20 : 0 })); await load() }
