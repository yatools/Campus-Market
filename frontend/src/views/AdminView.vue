...
async function decideMarketDispute(item: any, decision: 'completed' | 'cancelled') { const note = prompt('请输入交易纠纷裁决说明：', '') ?? ''; if (!note) return; await api(`/admin/market/disputes/${item.id}/decision`, json('POST', { decision, note })); success.value = '交易纠纷已裁决并写入审计日志'; await load() }
